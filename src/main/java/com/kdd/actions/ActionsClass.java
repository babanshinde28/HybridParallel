...
public class ActionsClass implements GlobalVariables{
	 ...
	 * 
	 * public void navigateToLoginScreen(String data) { homePage.clickSigninLink();
	 * }
	 * 
	 * public void navigateToShoppingCart(String data) { homePage.clickCartLink(); }
	 * 
	...
	public void login(List<Map<String,String>> data) {
		...
		System.out.println("data Value  :" + data.size());
		//New Added
		...
		{
			String userName=data.get(i).get("UserName");
			String password=data.get(i).get("Password");
			
			System.out.println("userName Counter :"+i+"-" + data.get(i).get("UserName"));
			System.out.println("password Counter :"+i+"-" + data.get(i).get("Password"));
			...
			assertTrue(loginPage.isSignOutLinkDisplayed(), "Login Successfully.");
	...
	//	loginToStore((String) SessionDataManager.getInstance().getSessionData("userName"),(String) SessionDataManager.getInstance().getSessionData("password"));
	//}
	...
	/*
	 * public void enterUserInformation(String data) { String[] dataArray =
	 * data.split("\\|"); String userName = dataArray[0]+System.nanoTime();
	 ...
	 * loginPage.enterUserName(userName); loginPage.enterPassword(dataArray[1]);
	 * registerationPage.enterConfirmPassowrd(dataArray[2]); }
	 * 
	 * public void enterAccountInformation(String data) { String[] dataArray =
	 * data.split("\\|"); registerationPage.enterFirstName(dataArray[0]);
	 ...
	 * if(dataArray[12].equalsIgnoreCase("yes")) { registerationPage.clickMyList();
	...
	/*
	 * public void searchForPet(String data) { homePage.searchPet(data);
	 * homePage.clickSearchButton(); assertTrue(homePage.isPageHeadingDisplayed(),
	 * "Search Results were not displayed"); }
	 * 
	 * public void selectAPet(String data) {
	 * productsPage.selectFirstPetFromSearchResult();
	 * SessionDataManager.getInstance().setSessionData("petDescription",
	 * productsPage.getPetDescritpion()); }
	 * 
	 * public void selectPetByID(String data) { productsPage.selectFirstPetID();
	 * assertEquals(SessionDataManager.getInstance().getSessionData("petDescription"
	 * ), productsPage.getPetPDPDescritpion(),
	 * "Pet Description did not match on PDP"); }
	 * 
	 * public void addToCart(String data) { productsPage.clickAddToCartButton();
	 * assertEquals(SessionDataManager.getInstance().getSessionData("petDescription"
	 * ), productsPage.getPetCartDescritpion(),
	 * "Pet Description did not match on Cart"); }
	 * 
	 * public void updateCart(String data) { cartPage.changeQuantity(data);
	 ...
	 * 
	 * public void confirmOrder(String data) { orderPage.clickConfirmButton(); }
	 * 
	 ...
	 * orderPage.getOrderNo(); assertEquals(orderPage.getDescription(),
	 * SessionDataManager.getInstance().getSessionData("petDescription"),
	 ...
	 * SessionDataManager.getInstance().setSessionData("orderId",
	 * orderPage.getFirstOrderID()); orderPage.selectFirstOrder(); }
	 * 
	 * public void deleteOrder(String data) { orderPage.clickDeleteOrderButton();
	 * assertTrue(homePage.isPageHeadingDisplayed(),
	 * "My Order page was not displayed"); }
	 * 
	 * public void validateOrderIsDeleted(String data) { String orderID = (String)
	 * SessionDataManager.getInstance().getSessionData("orderId");
	 * assertFalse(orderID.equalsIgnoreCase(orderPage.getFirstOrderID()),
	 * "Order was not deleted"); }
	 * 
	 * public void removePetFromCart(String data) throws InvalidLocatorException {
	 * int beforeRemove = cartPage.getPetsInCart(); cartPage.clickRemoveButton();
	 * assertEquals(cartPage.getPetsInCart(), beforeRemove-1,
	 * "Pet was not removed"); }
	 * 
	 * public void removeAllPetFromCart(String data) {
	 * cartPage.clickRemoveAllButton(); assertEquals(cartPage.getEmptyCartMessage(),
	 * data, "Cart was not empty"); }
	 * 
	 * public void selectPetCategory(String data) throws InvalidLocatorException {
	 * homePage.selectPetByCategory(data);
	 * assertTrue(homePage.isPageHeadingDisplayed(),
	 * data+" category was not displayed"); }
	 * 
	 * public void updatePaymentDetails(String data) { String[] dataArray =
	 * data.split("\\|"); billingDetailsPage.selectCardType(dataArray[0]);
	 * billingDetailsPage.updateCardNumber(dataArray[1]);
	 * billingDetailsPage.updateExpiryDate(dataArray[2]); }
	 * 
	 * public void updateBillingAddress(String data) { String[] dataArray =
	 * data.split("\\|"); billingDetailsPage.updateFirstName(dataArray[0]);
	 ...
	 * 
	 * public void updateShippingAddress(String data) { String[] dataArray =
	 * data.split("\\|"); shippingAddressPage.updateFirstName(dataArray[0]);
